...
const solve = (input: string): number => {
    ...
    const ruleMap: Map<string, string[][]> = new Map<string, string[][]>();
    sections[0].split('\n').forEach((line: string) => {
    ...
    const stack: any[][] = [];
    sections[1].split('\n').forEach((line: string) => {
        const categoryValues: string[][] = line.replace('{', '')
            .replace('}', '')
            .split(',')
            .map((categoryValue: string) => categoryValue.split('='));
        const categoryValueMap: Map<string, number> = new Map<string, number>(categoryValues.map((categoryValue: string[]) => [categoryValue[0], Number(categoryValue[1])]));
        stack.push(['in', categoryValueMap]);
    });
    let result: number = 0;
    while(stack.length > 0) {
        const [workflow, categoryValues] = stack.pop()!
        if(workflow === 'A' || workflow === 'R') {
            if(workflow === 'A')
                result += [...categoryValues.values()].reduce((a: number, b: number) => a + b, 0);
            continue;
        }
        const rules: string[][] = ruleMap.get(workflow)!;
        let matched: boolean = false, ruleIndex: number = 0;
        while(ruleIndex < rules.length && !matched) {
            if(rules[ruleIndex].length === 1) {
                stack.push([rules[ruleIndex][0], categoryValues])
                matched = true;
            } else {
                if(rules[ruleIndex][1] === '<' && categoryValues.get(rules[ruleIndex][0])! < Number(rules[ruleIndex][2])) {
                    stack.push([rules[ruleIndex][3], categoryValues]);
                    matched = true;
                }
                if(rules[ruleIndex][1] === '>' && categoryValues.get(rules[ruleIndex][0])! > Number(rules[ruleIndex][2])) {
                    stack.push([rules[ruleIndex][3], categoryValues]);
