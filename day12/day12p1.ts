...
const getPossibilities = (row: string, sections: number[], rowIndex: number, sectionIndex: number): number => {
    ...
    if(possibleSpring(row[rowIndex])) {
        ...
        if(rowIndex + sectionLength <= row.length) {
            ...
            if(validSectionPlacement(row, rowIndex, sectionLength) && possibleSprings.every(possibleSpring)) 
                result += getPossibilities(row, sections, rowIndex + sectionLength + 1, sectionIndex + 1);
        }
    }
    if(row[rowIndex] !== '#') result += getPossibilities(row, sections, rowIndex + 1, sectionIndex);
    return result;
}
...
const solve = (input: string): number => {
    ...
    .map((line: string[]) => getPossibilities(line[0], line[1].split(',').map(Number), 0, 0))
